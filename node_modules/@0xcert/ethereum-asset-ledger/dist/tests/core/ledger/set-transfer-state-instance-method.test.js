...
spec.test('grants ledger abilities for an account', (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    const ledger = ctx.get('ledger');
    ctx.true(yield ledger.isTransferable());
    const mutation = yield ledger.disableTransfers();
    yield mutation.complete();
    ctx.is((mutation.logs[0]).event, 'IsPaused');
    ctx.false(yield ledger.isTransferable());
    yield ledger.enableTransfers().then(() => ctx.sleep(200));
    ctx.true(yield ledger.isTransferable());
}));
exports.default = spec;
//# sourceMappingURL=set-transfer-state-instance-method.test.js.map