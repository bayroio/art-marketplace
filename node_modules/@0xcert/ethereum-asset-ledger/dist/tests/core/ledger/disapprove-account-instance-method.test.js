...
spec.test('disapproves account for token transfer', (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    const xcert = ctx.get('protocol').xcert;
    const bob = ctx.get('bob');
    const ledger = ctx.get('ledger');
    yield xcert.instance.methods.approve(bob, '1').send({});
    const mutation = yield ledger.disapproveAccount('1');
    yield mutation.complete();
    ctx.is((mutation.logs[0]).event, 'Approval');
    ctx.is(yield xcert.instance.methods.getApproved('1').call(), '0x0000000000000000000000000000000000000000');
}));
exports.default = spec;
//# sourceMappingURL=disapprove-account-instance-method.test.js.map