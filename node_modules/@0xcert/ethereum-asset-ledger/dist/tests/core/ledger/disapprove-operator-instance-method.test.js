...
spec.test('disapproves operator', (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    const xcert = ctx.get('protocol').xcert;
    const bob = ctx.get('bob');
    const coinbase = ctx.get('coinbase');
    const ledger = ctx.get('ledger');
    yield xcert.instance.methods.setApprovalForAll(bob, true).send({ from: coinbase });
    ctx.true(yield xcert.instance.methods.isApprovedForAll(coinbase, bob).call());
    const mutation = yield ledger.disapproveOperator(bob);
    yield mutation.complete();
    ctx.is((mutation.logs[0]).event, 'ApprovalForAll');
    ctx.false(yield xcert.instance.methods.isApprovedForAll(coinbase, bob).call());
}));
exports.default = spec;
//# sourceMappingURL=disapprove-operator-instance-method.test.js.map